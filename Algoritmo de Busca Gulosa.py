...
def bfs(origem, destino):
    ...
    while fila:
        ...
        for adjacente in vertice.adjacentes:
            if adjacente.vertice not in visitados:
                fila.append((adjacente.vertice, distancia + adjacente.custo))
...
# Solicitar ao usuário que insira o local atual e o destino
local_atual = input("Digite o local atual: ").lower()
local_destino = input("Digite o local de destino: ").lower()

# Procurar os vértices correspondentes a esses locais no grafo
vertice_atual = None
vertice_destino = None

for vertice in grafo.vertices:
    if vertice.rotulo.lower() == local_atual:
        vertice_atual = vertice
    elif vertice.rotulo.lower() == local_destino:
        vertice_destino = vertice

# Verificar se os vértices foram encontrados
if vertice_atual is None:
    print(f"O local '{local_atual}' não foi encontrado no grafo.")
    exit()
elif vertice_destino is None:
    print(f"O local '{local_destino}' não foi encontrado no grafo.")
    exit()

# Calcular a distância entre os locais usando o algoritmo BFS
distancia = bfs(vertice_atual, vertice_destino)

# Exibir a distância calculada
if distancia is not None:
    print(f"A distância entre {local_atual} e {local_destino} é: {distancia} km.")
else:
    print(f"Não há caminho entre {local_atual} e {local_destino}.")
